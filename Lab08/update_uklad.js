...
function startGL()
{
  ...
  `;
    ...
    textureImg9.src="neptune.png"; //Nazwa obrazka


    //Macierze opisujące położenie wirtualnej kamery w przestrzenie 3D
    let aspect = gl.viewportWidth/gl.viewportHeight;
    let fov = 45.0 * Math.PI / 180.0; //Określenie pola widzenia kamery
    let zFar = 4500.0; //Ustalenie zakresów renderowania sceny 3D (od obiektu najbliższego zNear do najdalszego zFar)
    let zNear = 0.1;
    uPMatrix = [
...
function Tick()
{
    ...
    for (let i = 0; i < 9; i++) {
        uMVRotY[i] = [
            +Math.cos(speed[i] * angleY * Math.PI / 180.0), 0, -Math.sin(speed[i] * angleY * Math.PI / 180.0), 0,
            0, 1, 0, 0,
            +Math.sin(speed[i] * angleY * Math.PI / 180.0), 0, +Math.cos(speed[i] * angleY * Math.PI / 180.0), 0,
            0, 0, 0, 1
        ];
    }

    let uMVRotYfast = [
        +Math.cos(190.0*angleY*Math.PI/180.0),0,-Math.sin(190.0*angleY*Math.PI/180.0),0,
        0,1,0,0,
        +Math.sin(190.0*angleY*Math.PI/180.0),0,+Math.cos(190.0*angleY*Math.PI/180.0),0,
        0,0,0,1
    ];

    let uMVRotY2 = [
        +Math.cos(23.0*angleY*Math.PI/180.0),0,-Math.sin(23.0*angleY*Math.PI/180.0),0,
        0,1,0,0,
        +Math.sin(23.0*angleY*Math.PI/180.0),0,+Math.cos(23.0*angleY*Math.PI/180.0),0,
        0,0,0,1
    ];

    let uMVRotY3 = [
        +Math.cos(20.0*angleY*Math.PI/180.0),0,-Math.sin(20.0*angleY*Math.PI/180.0),0,
        0,1,0,0,
        +Math.sin(20.0*angleY*Math.PI/180.0),0,+Math.cos(20.0*angleY*Math.PI/180.0),0,
        0,0,0,1
    ];

    let uMVRotY4 = [
        +Math.cos(2.0*angleY*Math.PI/180.0),0,-Math.sin(2.0*angleY*Math.PI/180.0),0,
        0,1,0,0,
        +Math.sin(2.0*angleY*Math.PI/180.0),0,+Math.cos(2.0*angleY*Math.PI/180.0),0,
        0,0,0,1
    ];

    let uMVRotYneptun = [
        +Math.cos(-1.5*angleY*Math.PI/180.0),0,-Math.sin(-1.5*angleY*Math.PI/180.0),0,
        0,1,0,0,
        +Math.sin(-1.5*angleY*Math.PI/180.0),0,+Math.cos(-1.5*angleY*Math.PI/180.0),0,
        0,0,0,1
    ];
    //bez przyspieszenia s
    let uMVRotYs = [
    ...
    for (let i= 0; i < 9; i++) {
        ...
        uMVMatrix[i] = MatrixMul(uMVMatrix[i], uMVRotY[i]);
    ...
    //Mercury
    gl.uniformMatrix4fv(gl.getUniformLocation(shaderProgram, "uMVMatrix"), false, new Float32Array(uMVMatrix[1]));
    gl.bindTexture(gl.TEXTURE_2D, textureBuffer2);
    gl.drawArrays(gl.TRIANGLES, 0, vertexPositionBuffer.numItems*vertexPositionBuffer.itemSize); //Faktyczne wywołanie rendrowania

    //Wenus
    gl.uniformMatrix4fv(gl.getUniformLocation(shaderProgram, "uMVMatrix"), false, new Float32Array(uMVMatrix[2]));
    gl.bindTexture(gl.TEXTURE_2D, textureBuffer3);
    gl.drawArrays(gl.TRIANGLES, 0, vertexPositionBuffer.numItems*vertexPositionBuffer.itemSize); //Faktyczne wywołanie rendrowania

    //Earth
    gl.uniformMatrix4fv(gl.getUniformLocation(shaderProgram, "uMVMatrix"), false, new Float32Array(uMVMatrix[3]));
    gl.bindTexture(gl.TEXTURE_2D, textureBuffer4);
    gl.drawArrays(gl.TRIANGLES, 0, vertexPositionBuffer.numItems*vertexPositionBuffer.itemSize); //Faktyczne wywołanie rendrowania

    //Mars
    gl.uniformMatrix4fv(gl.getUniformLocation(shaderProgram, "uMVMatrix"), false, new Float32Array(uMVMatrix[4]));
    gl.bindTexture(gl.TEXTURE_2D, textureBuffer5);
    gl.drawArrays(gl.TRIANGLES, 0, vertexPositionBuffer.numItems*vertexPositionBuffer.itemSize); //Faktyczne wywołanie rendrowania

    //Jowis
    gl.uniformMatrix4fv(gl.getUniformLocation(shaderProgram, "uMVMatrix"), false, new Float32Array(uMVMatrix[5]));
    gl.bindTexture(gl.TEXTURE_2D, textureBuffer6);
    gl.drawArrays(gl.TRIANGLES, 0, vertexPositionBuffer.numItems*vertexPositionBuffer.itemSize); //Faktyczne wywołanie rendrowania

    //Saturn
    gl.uniformMatrix4fv(gl.getUniformLocation(shaderProgram, "uMVMatrix"), false, new Float32Array(uMVMatrix[6]));
    gl.bindTexture(gl.TEXTURE_2D, textureBuffer7);
    gl.drawArrays(gl.TRIANGLES, 0, vertexPositionBuffer.numItems*vertexPositionBuffer.itemSize); //Faktyczne wywołanie rendrowania

    //Uranus
    gl.uniformMatrix4fv(gl.getUniformLocation(shaderProgram, "uMVMatrix"), false, new Float32Array(uMVMatrix[7]));
    gl.bindTexture(gl.TEXTURE_2D, textureBuffer8);
    gl.drawArrays(gl.TRIANGLES, 0, vertexPositionBuffer.numItems*vertexPositionBuffer.itemSize); //Faktyczne wywołanie rendrowania

    //Neptune
    gl.uniformMatrix4fv(gl.getUniformLocation(shaderProgram, "uMVMatrix"), false, new Float32Array(uMVMatrix[8]));
    gl.bindTexture(gl.TEXTURE_2D, textureBuffer9);
    gl.drawArrays(gl.TRIANGLES, 0, vertexPositionBuffer.numItems*vertexPositionBuffer.itemSize); //Faktyczne wywołanie rendrowania

    angleY=angleY+0.05;

    setTimeout(Tick,100.0);
}
